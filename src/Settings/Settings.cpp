...
//
// UniQ is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// UniQ is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with UniQ.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "uniq.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "stratum01.uniqnetwork.io:9992";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("uniqwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("uniqwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("uniqwallet.desktop");
  ...
  res = autorunSettings.contains("UniQWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("UniQWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.uniq.uniqwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=UniQ Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("UniQWallet", appPath);
    } else {
      autorunSettings.remove("UniQWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\uniq", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:uniq");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\uniq\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\uniq\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("uniqwallet.cfg"));
