...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("uniqcoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] UniQ wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "UniQ wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have uniqd with non-default RPC port, you should terminate it and relaunch UniQWallet\n"
      "or\n"
      "Set the Local deamon required port in UniQWallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
